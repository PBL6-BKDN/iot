...
GPS_LAST_FIX_FILE = Path(__file__).parent.parent / "data" / "gps_lastfix.json"
...
class GPSService:
    ...
    def _open_serial(self):
        for port in self._candidate_ports():
            try:
                if _serial_mod is None: raise RuntimeError("pyserial library not found")
                self.serial_port = _serial_mod.Serial(port=port, baudrate=BAUD_RATE, timeout=1)
    ...
    def run(self):
        """Chạy GPS service - tự động load vị trí cũ và cập nhật GPS thực"""
        # Service đã tự động khởi động thread trong __init__
        # Hàm này chỉ để giữ cho API rõ ràng
        logger.info("📡 GPS Service is running...")
        if self.current_lat:
            logger.info(f"📍 Current position: {self.current_lat}, {self.current_lng}")
        else:
...
if __name__ == "__main__":
    ...
    print("Đang lấy tín hiệu GPS thực tế và ghi log hành trình...")
    print("(Kiểm tra thư mục logs/gps_history)")
