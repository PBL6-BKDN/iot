#!/usr/bin/env python3
"""Monitored audio track for frame rate logging (simplified)."""
import asyncio
import time
...
class MonitoredAudioTrack(MediaStreamTrack):
    ...
    async def recv(self):
        ...
        if self.first_frame_ts is None:
            ...
            logger.info("First microphone audio frame captured")
        ...
        if now - self.last_log_ts >= self.log_interval:
            ...
            logger.info(f"Audio frames sent: {self.frame_count} ({fps:.1f} fps")
